...
def main():
    ...
    for idx in selected_indices:
        q = _fetch_question(api_url, auth, LAB, idx)
        if q is None:
            print(f"Question {idx} not found", file=sys.stderr)
            sys.exit(1)

        question = q["question"]
        question_num = selected_indices.index(idx) + 1

        # Run the agent
        data, error = _run_agent(question)

        if error:
            print(f"\n  {RED}x [{question_num}/{total_attempted}] {question}{RESET}")
            print(f"    Error: {error}")
            print(f"\n{BOLD}{passed}/{total_attempted} passed{RESET}")
            sys.exit(1)

        ok, reason = _check_question(q, data)

        if ok:
            print(f"  {GREEN}+ [{question_num}/{total_attempted}] {question}{RESET}")
            passed += 1
        else:
            answer = data.get("answer", "")
            print(f"\n  {RED}x [{question_num}/{total_attempted}] {question}{RESET}")
    ...
    # All questions done
    print(f"\n{BOLD}{GREEN}{passed}/{total_attempted} PASSED{RESET}")
    print(
